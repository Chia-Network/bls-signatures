 ...
 * and corrects the result by conditionally adding 2^(FP_DIGS * WORD_SIZE) * p.
 * Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first field element to add.
 ...
 * 2^(FP_DIGS * WORD_SIZE) * p. Computes c = a - b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first field element to add.
 * @param[in] b				- the second field element to add.
 ...
 * the result by adding (2^(FP_DIGS * WORD_SIZE) * p)/4. This function should only
