...
// G1Element
int CG1ElementSize() { return bls::G1Element::SIZE; }

CG1Element CG1ElementFromBytes(const void* data, bool* didErr)
{
    bls::G1Element* el = nullptr;
    try {
        el = new bls::G1Element(bls::G1Element::FromBytes(
            bls::Bytes((uint8_t*)(data), bls::G1Element::SIZE)));
...
// G2Element
int CG2ElementSize() { return bls::G2Element::SIZE; }

CG2Element CG2ElementFromBytes(const void* data, bool* didErr)
{
    bls::G2Element* el = nullptr;
    try {
        el = new bls::G2Element(bls::G2Element::FromBytes(
            bls::Bytes((uint8_t*)data, bls::G2Element::SIZE)));
        *didErr = false;
    } catch (const std::exception& ex) {
        fErrMsg = ex.what();
        *didErr = true;
